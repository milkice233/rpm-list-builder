...
with open('./python37.yaml') as f:
    pkgs = yaml.load(f)['python37']['packages']
...
for pkg in pkgs:
    ...
    chaneglog = 'Rebuilt for Python 3.7'
    if isinstance(pkg, dict):
        assert len(pkg) == 1
        chaneglog = 'Bootstrap for Python 3.7'
    ...
    r(f'sleep 3')
    r(f'git push')
    r(f'fedpkg build --target=f29-python || :')
    r(f'while ! koji wait-repo f29-python --build='
      f'$(rpm --define \'_sourcedir .\'  --define \'dist .fc29\' -q '
      f'--qf "%{{NAME}}-%{{VERSION}}-'
